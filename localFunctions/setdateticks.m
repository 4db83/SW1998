function step_sizes_out = setdateticks(dates,Width,date_format,FNTS,date_space,fig_handle)
...
SetDefaultValue(3, 'date_format', 0);
...
% 	if dateformat == 0      || strcmpi(dateformat,'q')
% 		dateformat = 'yyyy:QQ';
% 	elseif dateformat == 1  || strcmpi(dateformat,'m')
% 		dateformat = 'mmm-yyyy';
% 	elseif dateformat == 2  || strcmpi(dateformat,'d')
% 		dateformat = 'dd.mm.yyyy';
% 	end
% end

if length(date_format) == 1
	if date_format == 0      || strcmpi(date_format,'q')
		date_format = 'yyyy:QQ';
	elseif date_format == 1  || strcmpi(date_format,'m')
		date_format = 'mmm-yyyy';
	elseif date_format == 2  || strcmpi(date_format,'d')
...
if ~isfloat(dates)
	...
	set(fig_handle,'XTick'			, DT);
	if Imin_x
		set(fig_handle,'XTicklabel'	, datestr(dates(DT), date_format))
	else
		set(fig_handle,'XTicklabel'	, datestr(DT, date_format))
	end
	set(fig_handle,'FontName'		, FName)
end
