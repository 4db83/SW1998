function YX_Stage2 = make_yx_4_stage2_unrestricted(YY, XX, Stage2_KFS_object)
% Function: makes the Y and X variables needed for HLWs Stage 2 MUE inputs in unrestricted from.
% ---------------------------------------------------------------------------------------------
...
% where - YY =	[y~(t)]
% and 
%				- XX =	[y~(t-1) y~(t-1) r(t-1) r(t-2) g(t-1) 1] (1 is an intercept term), 
%								g(t) NOT annualized.
% db
% --------------------------------------------------------------------------------------------- 
...
% note: xi(t) = [y*(t) y*(t-1) y*(t-2) g(t-1)], so take the first two row entries in y*(t-2) and stack on top of y*(t) 
...
% X = [y~(t-1) y~(t-1) r(t-1) r(t-2) g(t-1) 1]
