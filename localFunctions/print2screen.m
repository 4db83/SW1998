...
% OR		 : print2screen(y). if y is a Table or TimeTable, second input is the format
% ---------------------------------------------------------------------------------------------------
...
sep
FMT_0 = '%10.4f';
if nargin == 1; FMT = FMT_0; RowNames_in = FMT_0; end

structure_in	= isa(y,'struct');
table_in			= isa(y,'table');
timetable_in	= isa(y,'timetable');

if table_in | timetable_in % if Table or Timetable       
  ColNames_tmp  = y.Properties.VariableNames(1:end)'; 
  FMT           = RowNames_in;
end
